...
export function Services() {
  ...
  const handleUpdateCategory = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const categoryRef = doc(db, "servicesCategory", editingCategory.id!);
  ...
  const handleDeleteCategory = async (
    ...
    const servicesInCategory = await getServicesByCategoryFlat(categoryName);

    if (servicesInCategory.length > 0) {
      alert(
        `Cannot delete "${categoryName}" - it has ${servicesInCategory.length} service(s). Please delete or move the services first.`
      );
      return;
    }
  ...
  const handleMigration = async () => {
    if (
      !confirm(
        "⚠️ This will migrate all services to the new structure. Continue?"
      )
    ) {
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingService) {
        await (useNewStructure ? updateServiceFlat : updateServiceFlat)(
          serviceData
        );
      } else {
        await (useNewStructure ? addServiceFlat : addServiceFlat)(serviceData);
      }
  ...
  const handleDeleteService = async (service: Service) => {
    ...
    try {
      setLoading(true);
      await deleteServiceFlat(service.id!); // Always use flat structure now
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 text-white px-6 py-3 rounded-lg font-semibold hover:from-blue-700 hover:to-blue-800 transition-all shadow-lg"
            >
              <Folder size={20} />
            ...
            <button
              onClick={() => {
                const current =
                  localStorage.getItem("useNewStructure") === "true";
        ...
        {totalServices > PAGE_SIZE && (
          <div className="flex justify-between items-center mt-6 pt-6 border-t">
            <div className="text-sm text-gray-600">
              ...
              {Math.min(currentPage * PAGE_SIZE, totalServices)} of{" "}
